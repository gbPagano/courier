...
| Kind | Use when |
| ---- | -------- |
| [`api`](#api) | Sending envelopes to an HTTP endpoint. |
| [`file`](#file) | Appending envelopes to a local JSONL or CSV file. |
| [`kafka`](#kafka) | Producing records to a Kafka topic. |
| [`sql`](#sql) | Inserting envelopes into a SQL table. |
...
| `method`       | no       | `"POST"`    | Any HTTP method understood by `reqwest::Method`, for example `POST`, `PUT`, `PATCH`, or `DELETE`. |
